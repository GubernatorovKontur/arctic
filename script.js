...
function initBackgroundMusic() {
    ...
    let isMuted = localStorage.getItem('arcticExpeditionMuted') === 'true';
    ...
    if (audio) {
        ...
        // Handle user interaction to enable audio (only if not muted and audio is paused)
        if (!isMuted && audio.paused) {
            document.addEventListener('click', function(event) {
                // Не включаем звук при клике на кнопку mute
    ...
    if (muteButton && audio) {
        muteButton.addEventListener('click', function() {
            ...
            localStorage.setItem('arcticExpeditionMuted', isMuted.toString());
            ...
            if (isMuted) {
                ...
                muteButton.title = 'Отключить звук';
